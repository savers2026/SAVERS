%% ************************************************************************
% MODULE NAME:      write_L1a_blackbodyzenith.m
...
%% ************************************************************************
% FUNCTION
% The module writes the blackbodyZenith.nc L1a product of the Hydrognss
% E2ES.
%% ************************************************************************
% REFERENCE
...
function write_L1a_bbZenith(IntegrationMidPointTime, geoSYSp, Rx, DDMOutputNumericalScaling_NLblackbody_UPmax, DDMPixelValueNoiseWholeDDM_up,...
                           DDM_NLBB_up, PRN, outpathname, foldername)
...
FrontEnd                             = repmat(4,size(IntegrationMidPointTime));
...
TrackingOffsetDelayInPixels          = repmat(geoSYSp.delayOffset_bin,size(IntegrationMidPointTime));
...
DDMOutputNumericalScaling  = DDMOutputNumericalScaling_NLblackbody_UPmax;
DDMPixelValueNoiseWholeDDM = DDMPixelValueNoiseWholeDDM_up;
maxddm                     = max(max(DDM_NLBB_up));
ddm1                       = zeros(size(DDM_NLBB_up));

for ki=1:l
    ddm1(:,:,ki)= ((DDM_NLBB_up(:,:,ki).*65535./(maxddm(ki))));
...
ncid  = netcdf.create(strjoin([outpathname '\' 'blackbodyZenithL1E1.nc'],''),'netcdf4');
...
netcdf.putAtt(ncid, netcdf.getConstant("NC_GLOBAL"),'Name','HydroGNSS L1a blackbodyZenithL1E1.nc');
...
netcdf.putAtt(ncid, netcdf.getConstant("NC_GLOBAL"),'BlackbodyType',uint32(0));
netcdf.close(ncid);
