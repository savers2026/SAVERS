function [SP_lla, SP_ECEF] = Specular_Point_Position_Calculation_PlusMeanElevation(Pos_GPS, Pos_Rec,meanSurfaceElevation)
%--------------------------------------------------------------------------
% [SP_lla, SP_ECEF] = Specular_Point_Position_Calculation(Pos_Rec, Pos_GPS, meanSurfaceElevation)
%--------------------------------------------------------------------------
% Function Name:  Specular_Point_Position_Calculation.m
% Author:         Starlab Barcelona SL

% Last Update:    October 2017 by Laura Dente, adapted to find the specular
%                 point over a larger ellipsoid. The axis of the earth ellipsoid are
%                 increased of a quantity equal to the local surface elevation of the area
%                 of interest.
%                 March 2023 by Laura Dente, minor axis is obtained from
%                 the major axis and the flattening
%--------------------------------------------------------------------------
%
...
% plus a local mean surface elevation, obtained from the local DEM.
% The specular point position is obtained as the point where the distance
% emitter-point-receiver is minimized.
...
% meanSurfaceElevation   -  mean elevation of the surface observed by the
%                           emitter, DEM product (m) 
% Output
% ------
...
% WGS84 Earth Ellipsoid plus the DEM elevation
earthMajax = 6.37813700000000e6+meanSurfaceElevation;
f = 1/298.257223563;
%earthMinax = 6.356752314140356e6+meanSurfaceElevation;
earthMinax = (1-f)*6.37813700000000e6 + meanSurfaceElevation;

[SP_lla,SP_ECEF,inds] = findSpecularPoint(Pos_GPS,Pos_Rec,earthMajax,earthMinax);
...
function [R_lla,R_ECEF,inds] = findSpecularPoint(emitter,receiver,earthMajax,earthMinax)
%--------------------------------------------------------------------------
% [R_lla,R_ECEF,inds] = findSpecularPoint(emitter,receiver,model)
%--------------------------------------------------------------------------
% Function Name:  findSpecularPoint.m
% Author:         Starlab Barcelona SL
% Last Update:    2010/10/21
% Modified by Laura Dente in October 2017
%--------------------------------------------------------------------------
%
% Finds the specular reflection point on earth surface for a given emitter and 
% receiver location. Assumes an ellipsoidal Earth, model WGS84.An
% additional elevation is added the ellipsoid to take into account of the
% local DEM.
...
for j=1:num
    ...
    if rayIntersectsEarth(emitter(j,:),receiver(j,:),earthMajax,earthMinax)
        if j<num-1
        ...
        [res,feval,exitflag,output] = fminsearch(@costFunction,firstGuess,...
            options,emitter(j,:),receiver(1,:),earthMajax,earthMinax);
        ...
        if exitflag>0
            ...
            r = earthSurfacePoint(res(1),res(2),earthMajax,earthMinax);
            R(at,:)=r;
...
earthMajaxWGS84 = 6.37813700000000e6;
f=1/298.257223563;
% earthMinaxWGS84 = 6.356752314140356e6;
% f = 1-earthMinaxWGS84/earthMajaxWGS84;
tol = 10e-10;
...
phidk = delta;
while 1
    rck = earthMajaxWGS84*sqrt((1-2*f+f*f)./(1-(2*f-f*f)*cos(phidk).^2));
...
function b = rayIntersectsEarth(p,q,earthMajax,earthMinax)
%--------------------------------------------------------------------------
...
function c = costFunction(pos,emitter,receiver,earthMajax,earthMinax)
%--------------------------------------------------------------------------
...
x=earthSurfacePoint(ascension,declination,earthMajax,earthMinax);
d1 = emitter-x;
d2 = receiver-x;
c = norm(d1)+norm(d2);
...
function v = earthSurfacePoint(ascension,declination,earthMajax,earthMinax)
%--------------------------------------------------------------------------
...
s = sphere2rect(p);
r = earthRadiusInDirection(s,earthMajax,earthMinax);
v = r*s;
...
function r = earthRadiusInDirection(v,earthMajax,earthMinax)
%--------------------------------------------------------------------------
