...
coarse_step = 5;
beta0_vals_coarse = 0.0001:coarse_step:100;
beta2_vals_coarse = 0.0001:coarse_step:100;

RMSE_matrix = zeros(length(beta0_vals_coarse), length(beta2_vals_coarse));
Pr_matrix = zeros(length(beta0_vals_coarse), length(beta2_vals_coarse));
coeff_matrix_all = cell(length(beta0_vals_coarse), length(beta2_vals_coarse));

for i = 1:length(beta0_vals_coarse)
    for j = 1:length(beta2_vals_coarse)
        [pr_scalar, coeff_matrix] = radarEquation(R0, Rs, lambda, sigma_z, ...
            ...
            beta0_vals_coarse(i), beta2_vals_coarse(j), theta_i, phi_i, ...
...
%% Step 2: Find best coarse region
[min_val, linear_idx] = min(RMSE_matrix(:));
[idx_i, idx_j] = ind2sub(size(RMSE_matrix), linear_idx);
beta0_best_coarse = beta0_vals_coarse(idx_i);
beta2_best_coarse = beta2_vals_coarse(idx_j);

%% Step 3: Fine grid search around best coarse point
fine_step = 0.1;
range_half = coarse_step;  % ±coarse_step around best coarse value

% Ensure positive beta values only
beta0_start = max(beta0_best_coarse - range_half, 0.001);
beta0_end   = beta0_best_coarse + range_half;
beta2_start = max(beta2_best_coarse - range_half, 0.001);
beta2_end   = beta2_best_coarse + range_half;

beta0_vals_fine = beta0_start:fine_step:beta0_end;
beta2_vals_fine = beta2_start:fine_step:beta2_end;

RMSE_matrix = zeros(length(beta0_vals_fine), length(beta2_vals_fine));
Pr_matrix = zeros(length(beta0_vals_fine), length(beta2_vals_fine));
coeff_matrix_all = cell(length(beta0_vals_fine), length(beta2_vals_fine));

for i = 1:length(beta0_vals_fine)
    for j = 1:length(beta2_vals_fine)
        [pr_scalar, coeff_matrix] = radarEquation(R0, Rs, lambda, sigma_z, ...
            [], Pt, Ny_surf, Nx_surf, TRAp_Betac.TxFrequency, ...
            SMvol, sigma_z_cm, R0_slant, Rs_slant, ...
            beta0_vals_fine(i), beta2_vals_fine(j), theta_i, phi_i, ...
            theta_s_vector, phi_s_vector, surfParam, TRAp_Betac, sigmaParam_Betac);
...
%% Step 4: Get best beta0 and beta2 from fine search
[min_val, linear_idx] = min(RMSE_matrix(:));
[idx_i, idx_j] = ind2sub(size(RMSE_matrix), linear_idx);
beta0_opt = beta0_vals_fine(idx_i);
beta2_opt = beta2_vals_fine(idx_j);

beta_opt = [beta0_value, beta2_value];
