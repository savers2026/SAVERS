...
% Rs_range = [Rs_org - 1000 , Rs_org, Rs_org + 1000];
...
Pr_Rs_Theta_beta2_matrix = zeros(num_rs, num_theta);
P_IT_optBeta_Theta =  zeros(num_rs, num_theta);

for Rs_idx = 1:length(Rs_range)
...
[~,~,R1_scalar] = xyz2llh(TRAp_Betac.TxECEF(1),TRAp_Betac.TxECEF(2),TRAp_Betac.TxECEF(3));
  ...
  R1   = repmat(R1_scalar ,Ny_surf,Nx_surf,1); %check
...
Cords.RxENU_Rs = TRAp_Betac.TxECEF;
...
pr_matrix_optBeta = db(P_IT_matrix_all(:,idx_best), 'power');
P_IT_optBeta = db(Pr_Theta_beta2_matrix(:,idx_best), 'power');
...
title(sprintf('p_r and P_{IT} vs. Incidence angle at Optimal \\beta_2 (H: %d m)', round(Rs)), ...
    'FontSize', 10);
legend('Location', 'best', 'FontSize', 10);
set(gca, 'FontSize', 10);

Pr_Rs_Theta_beta2_matrix(Rs_idx, :) = pr_matrix_optBeta;
P_IT_optBeta_Theta(Rs_idx,:) = P_IT_optBeta;
