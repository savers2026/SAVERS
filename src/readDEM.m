%% ************************************************************************
% MODULE NAME:      readDEM.m
% SOFTWARE NAME:    HSAVERS
% SOFTWARE VERSION: 2.5
%% ************************************************************************
% AUTHORS:      L. Dente
% @copyright:   Tor Vergata University of Rome
% Original version:      2019 by Laura Dente
% Main updates:          2022 by L. Dente
% Released to ESA:       December 2022
%% ************************************************************************
% FUNCTION
% The module opens a DEM in geotiff format or .mat format (obtained by a mosaic
% of SRTM DEM images) and reads only a specific area of the image around the SP and defined
% by the area size given as input. The DEM must be referred to WGS84.
%% ************************************************************************
% REFERENCE
% HydroGNSS E2E Simulator User Guide 
% HydroGNSS E2E Simulator Algorithm Theoretical Baseline Document
%% ************************************************************************

function [coordDEM_ecef,DEM_lla_resized,deltaLatDEM,deltaLonDEM] = readDEM(DEMinfo,coord_SP_lla_onEllipsoid, flagFlooding)

if contains(DEMinfo.filenameDEM, 'tif')

    % if the surface was set as flooded in the classes 5 and 6 of the cover
    % map (i.e. flooded forest and flooded low vegetation) then the
    % modified DEM will be used (DEM were the flooded classe and water
    % bodies are set to a constant elevation)
    if flagFlooding==1
        filenameDEM = DEMinfo.filenameDEM_flooded;
    else
        filenameDEM = DEMinfo.filenameDEM;
    end

    %read the surface elevation data
    [surfaceElevation_zero, spatialRefInfo]=readgeoraster(filenameDEM);
    %latitude delta of DEM
    deltaLatDEM = spatialRefInfo.CellExtentInLatitude;
    %longitude delta of DEM
    deltaLonDEM = spatialRefInfo.CellExtentInLongitude;

    %coordinates of the upper left corner of the upper left pixel
    maxLatOrigDEM=spatialRefInfo.LatitudeLimits(2);
    minLonOrigDEM=spatialRefInfo.LongitudeLimits(1);
    %coordinates of the bottom right corner of the bottom right pixel
    minLatOrigDEM=spatialRefInfo.LatitudeLimits(1);
    maxLonOrigDEM=spatialRefInfo.LongitudeLimits(2);
    %DEM size
    rasterWidth=spatialRefInfo.RasterSize(2);
    rasterHeight=spatialRefInfo.RasterSize(1);
    %array of lat and lon of original DEM
    %these are the coordinates of the center of the pixel
    latDEM_zero=(minLatOrigDEM+deltaLatDEM/2):deltaLatDEM:(maxLatOrigDEM-deltaLatDEM/2);
    lonDEM_zero=(minLonOrigDEM+deltaLonDEM/2):deltaLonDEM:(maxLonOrigDEM-deltaLonDEM/2);
    %when ENVI makes the mosaic, it introduces a column of zeros on the right of
    %the image and a line of zeros at the bottom of the image.
    %These column and line are here removed from the lat and lon array
    latDEM=latDEM_zero(2:rasterHeight);
    lonDEM=lonDEM_zero(1:rasterWidth-1);
    latDEM=flipud(latDEM');
    
    clear latDEM_zero
    clear lonDEM_zero
    %the column and line of zeros introduced by ENVI are removed
    surfaceElevation=surfaceElevation_zero(1:rasterHeight-1,1:rasterWidth-1);
    clear surfaceElevation_zero

    %compute matrix of LAT
    DEM_lla(:,:,1)=repmat(latDEM,1,length(lonDEM));
    %compute matrix of LON
    DEM_lla(:,:,2)=repmat(lonDEM,length(latDEM),1);
    DEM_lla(:,:,3) = surfaceElevation;
    clear surfaceElevation

else
    load(DEMinfo.filenameDEM)
    deltaLatDEM=abs(DEM_lla(1,1,1)-DEM_lla(2,1,1));
    deltaLonDEM=abs(DEM_lla(1,1,2)-DEM_lla(1,2,2));
end
% define the borders to resize the DEM
% Note: the DEM area should be large enough to include the complete SP
% track selected and to include for all SP the integration area, which is up
% to 450 km x 450 km around SP (for an incidence angle of 50°) and which
% is rotated of theta.
maxLatDEM=coord_SP_lla_onEllipsoid(1)+DEMinfo.sizeInputMapWindow;
minLatDEM=coord_SP_lla_onEllipsoid(1)-DEMinfo.sizeInputMapWindow;
maxLonDEM=coord_SP_lla_onEllipsoid(2)+DEMinfo.sizeInputMapWindow;
minLonDEM=coord_SP_lla_onEllipsoid(2)-DEMinfo.sizeInputMapWindow;
areaDEM=[minLatDEM maxLatDEM minLonDEM maxLonDEM];

%resize the DEM
indexLatDEMresized= find(DEM_lla(:,1,1)>=areaDEM(1) & DEM_lla(:,1,1)<=areaDEM(2));
indexLonDEMresized=find(DEM_lla(1,:,2)>=areaDEM(3) & DEM_lla(1,:,2)<=areaDEM(4));
%array of lat and lon of the selected part of the DEM
DEM_lla_resized(:,:,1)=DEM_lla(indexLatDEMresized,indexLonDEMresized,1);
DEM_lla_resized(:,:,2)=DEM_lla(indexLatDEMresized,indexLonDEMresized,2);
DEM_lla_resized(:,:,3)=DEM_lla(indexLatDEMresized,indexLonDEMresized,3);
clear DEM_lla
clear indexLatDEMresized
clear indexLonDEMresized


%% --- set to zero the DEM elevation for a test without topography --- 
% after the transformation in ENU the DEM will follow the Earth surface
% curvature. To have a flat surface without topography and without
% curvature, the ENU z coordinate should be set to zero. 
if DEMinfo.flatsurface==1
    DEM_lla_resized(:,:,3) = 0.0;
end

%% ------------------------------------------------------

%convert DEM coordinates from geodetic to ECEF
wgs84 = wgs84Ellipsoid('meters');

[coordDEM_ecef(:,:,1), coordDEM_ecef(:,:,2), coordDEM_ecef(:,:,3)] =geodetic2ecef(deg2rad(DEM_lla_resized(:,:,1)),deg2rad(DEM_lla_resized(:,:,2)),double(DEM_lla_resized(:,:,3)),wgs84);

return
