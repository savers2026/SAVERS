...
%% ************************************************************************
 ...
 function []=START_Simulator(filenamedefault)
%% starting counter
...
tag.SNR            = 0; %1 to compute the SNR and save it in the inOutReferenceFile
...
if tag.PattInterp == 1
    nadirPattern_offPointing_new  =  0:configParam.dang:90;
...
if tag.debug == 0
    ...
    [Tx, VTx, Rx, VRx, outputFilename, SPlatlonAlt, incAngle, PRN, GNSS_Week, GNSS_SoW,TrackID,SVN,goToSimulation,OrbitWithReflectivity,ReflectivitydB_selected,tag,Rx_roll,Rx_pitch,Rx_yaw]...
...
if goToSimulation == 1
    ...
    if tag.varibleBioGeoInputs==0
        [landCoverMap,coverMap_IDs,terrainParameters,vegetationParameters] = readCoverClassesAndDefineParam(tag,filenameCoverMap, SPlatlonAlt, DEMinfo, bioGeoInputs, presentLogInputsFilename);
    else
        [landCoverMap,coverMap_IDs,terrainParameters,vegetationParameters,descreteVariableParam,randomVariableParameter] = readCoverClassesAndDefineVariableParam(tag,filenameCoverMap, SPlatlonAlt, DEMinfo,tag.scenario, bioGeoInputsVariable, presentLogInputsFilename);
    end
