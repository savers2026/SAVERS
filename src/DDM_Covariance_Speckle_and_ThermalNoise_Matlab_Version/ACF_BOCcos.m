function [CodePhase, ACF, sampling_rate, range_delay] = ACF_BOCcos(chip_rate, sampling_rate, length_in_chip, m, n)
    ...
    for k = 1-alpha:alpha-1
        for j = 1:length(code_phase)
            t=code_phase(j);
            tmp1(j) = 6.0*Tri(t*alpha-2.0*k,1.0)+Tri(t*alpha-2.0*k-1.0,1.0)+Tri(t*alpha-2.0*k+1.0,1.0)-4.0*Tri(t*alpha-2.0*k+1.0/2.0,1.0)-4.0*Tri(t*alpha-2.0*k-1.0/2.0,1.0);
        end
